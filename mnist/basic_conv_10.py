...
working_params = hyper_params["basic_conv_10"]
# working_params
...
def convolution_layer(input_layer, weight_shape, bias_shape, activation = tf.nn.relu):
    ...
    w = tf.Variable(tf.random_normal(weight_shape), dtype=tf.float32)
    b = tf.Variable(tf.random_normal(bias_shape), dtype=tf.float32)
...
with g.as_default():
    ...
    with tf.name_scope("convolution_1") as scope:
        ...
        convolution_layer_1 = convolution_layer(x_input_reshape, [5, 5, 1, 64], [64],
    ...
    with tf.name_scope("convolution_2") as scope:
        ...
        convolution_layer_2 = convolution_layer(pooling_layer_1, [5, 5, 64, 128], [128],
                                                    activation = tf.nn.sigmoid)
    ...
    with tf.name_scope("convolution_3") as scope:
        ...
        convolution_layer_3 = convolution_layer(pooling_layer_2, [5, 5, 128, 256], [256],
                                                    activation = tf.nn.sigmoid)
    ...
    with tf.name_scope("flatten_pool") as scope:
        pooling_shape = pooling_layer_3.get_shape().as_list()
        flattened_pool = tf.reshape(pooling_layer_3, [-1, pooling_shape[1] * pooling_shape[2] * pooling_shape[3]])

    with tf.name_scope("bottleneck") as scope:
        dense_layer_bottleneck = dense_layer(flattened_pool, 2048)
