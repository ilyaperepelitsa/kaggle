...
working_params = hyper_params["basic_conv_17"]
# working_params
...
def convolution_layer(input_layer, weight_shape, bias_shape, activation = tf.nn.relu):
    ...
    w = tf.Variable(tf.zeros(weight_shape), dtype=tf.float32)
    b = tf.Variable(tf.zeros(bias_shape), dtype=tf.float32)
