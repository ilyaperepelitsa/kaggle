...
working_params = hyper_params["basic_conv_22"]
# working_params
...
with g.as_default():
    ...
    with tf.name_scope("logits") as scope:
        logits = dense_layer(dropout_layer, no_classes, activation = tf.nn.softmax)
    ...
    with tf.name_scope("loss") as scope:
        ...
        loss_operation = tf.reduce_mean(loss, name="loss")
    ...
    # session = tf.Session()
        ...
        for step in range(working_params["num_steps"]):
            ...
            # print(session.run(dense_layer_bottleneck, feed_dict={
