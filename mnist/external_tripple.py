...
working_params = hyper_params["external_tripple"]
...
with g.as_default():
    ...
    with tf.name_scope("stack_1") as scope:
        ...
        conv_1 = conv_layer(x_input_reshape, shape = [5, 5, 1, 64])
        conv_1_pool = max_pool_2x2(conv_1)

        conv_2 = conv_layer(conv_1_pool, shape = [5, 5, 64, 128])
        conv_2_pool = max_pool_2x2(conv_2)

        conv_3 = conv_layer(conv_2_pool, shape = [5, 5, 128, 256])
        conv_3_pool = max_pool_2x2(conv_3)

        # conv_2_flat = tf.reshape(conv_2_pool, [-1,])
        pooling_shape = conv_3_pool.get_shape().as_list()
        conv_2_flat = tf.reshape(conv_3_pool, [-1, pooling_shape[1] * pooling_shape[2] * pooling_shape[3]])


    with tf.name_scope("stack_2") as scope:

        conv_1_1 = conv_layer(x_input_reshape, shape = [5, 5, 1, 64])
        conv_1_1_pool = avg_pool_2x2(conv_1_1)

        conv_2_1 = conv_layer(conv_1_1_pool, shape = [5, 5, 64, 128])
        conv_2_1_pool = avg_pool_2x2(conv_2_1)

        conv_3_1 = conv_layer(conv_2_1_pool, shape = [5, 5, 128, 256])
        conv_3_1_pool = avg_pool_2x2(conv_3_1)

        # conv_2_flat = tf.reshape(conv_2_pool, [-1,])
        pooling_shape_1 = conv_3_1_pool.get_shape().as_list()
        conv_2_1_flat = tf.reshape(conv_3_1_pool, [-1, pooling_shape_1[1] * pooling_shape_1[2] * pooling_shape_1[3]])

    with tf.name_scope("stack_3") as scope:

        conv_1_2 = conv_layer(x_input_reshape, shape = [3, 3, 1, 64])
        conv_1_2_pool = avg_pool_2x2(conv_1_2)

        conv_2_2 = conv_layer(conv_1_2_pool, shape = [3, 3, 64, 128])
        conv_2_2_pool = avg_pool_2x2(conv_2_2)

        conv_3_2 = conv_layer(conv_2_2_pool, shape = [3, 3, 128, 256])
        conv_3_2_pool = avg_pool_2x2(conv_3_2)

        # conv_2_flat = tf.reshape(conv_2_pool, [-1,])
        pooling_shape_2 = conv_3_2_pool.get_shape().as_list()
        conv_2_2_flat = tf.reshape(conv_3_2_pool, [-1, pooling_shape_2[1] * pooling_shape_2[2] * pooling_shape_2[3]])

    with tf.name_scope("stack_4") as scope:

        conv_1_3 = conv_layer(x_input_reshape, shape = [3, 3, 1, 64])
        conv_1_3_pool = max_pool_2x2(conv_1_3)

        conv_2_3 = conv_layer(conv_1_3_pool, shape = [3, 3, 64, 128])
        conv_2_3_pool = max_pool_2x2(conv_2_3)

        conv_3_3 = conv_layer(conv_2_3_pool, shape = [3, 3, 128, 256])
        conv_3_3_pool = max_pool_2x2(conv_3_3)

        pooling_shape_3 = conv_3_3_pool.get_shape().as_list()
        conv_2_3_flat = tf.reshape(conv_3_3_pool, [-1, pooling_shape_3[1] * pooling_shape_3[2] * pooling_shape_3[3]])
